...
_SHARED_CODE_RULESETS = {"mysql_rules.json", "postgresql_rules.json", "mssql_rules.json", "oracle_rules.json"}
...
def diff_records(rejudge_results):
    ...
    for r in rejudge_results:
        ...
        if not record.get("code"):
            ...
            detail_note = "동일 코드가 여러 룰셋(MySQL/PostgreSQL/MSSQL/Oracle)에 존재하고 (CMD)로도 구분할 수 없습니다."
